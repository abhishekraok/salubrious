...
class InvestmentPolicy(Base):
    ...
    expected_years_remaining: Mapped[Optional[int]] = mapped_column(nullable=True)
    expected_years_earning: Mapped[Optional[int]] = mapped_column(nullable=True)
    expected_after_tax_salary: Mapped[Optional[float]] = mapped_column(nullable=True)
    withdrawal_rate_pct: Mapped[float] = mapped_column(default=3.5)
