...
def _get_allocation(user: UserProfile, db: Session):
...
@router.get("/current")
def get_current_allocation(user: UserProfile = Depends(get_current_user), db: Session = Depends(get_db)):
    allocation, _ = _get_allocation(user, db)
...
def get_suggested_actions(
    ...
    allocation, policy = _get_allocation(user, db)
